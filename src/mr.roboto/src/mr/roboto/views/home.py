    ...
    if line_parsed:
        ...
        if pull_request_log:
            ...
            if commit_log:
                ...
                msg = msg.replace(
                    commit_log_groups[1], COMMIT_URL.format(*commit_log_groups)
                )
...
def log_page(context, request):
    ...
    return {'success': True, 'file_size': file_size, 'log': log}
...
@view_config(
    route_name='missing_changelog', renderer='mr.roboto:templates/missing_changelog.pt'
)
def missing_changelog(context, request):
    return {}


@view_config(route_name='branches', renderer='mr.roboto:templates/branches.pt')
