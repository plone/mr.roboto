...
@mock.patch("mr.roboto.buildout.PloneCoreBuildout.commit_changes")
def test_not_in_checkouts(_commit_changes, _push_changes, _clone, caplog):
    ...
    assert len(caplog.records) == 1
    assert (
        "add plone.uuid to checkouts.cfg of buildout.coredev 5.0"
        in caplog.records[0].msg
    )
...
@mock.patch("mr.roboto.buildout.PloneCoreBuildout.push_changes")
@mock.patch("mr.roboto.buildout.PloneCoreBuildout.commit_changes")
def test_not_in_multiple_checkouts(_commit_changes, _push_changes, _clone, caplog):
    ...
    assert (
        "add plone.uuid to checkouts.cfg of buildout.coredev 4.3"
